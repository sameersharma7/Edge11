        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block text-sm text-slate-400">
              ID: <span className="text-emerald-400 font-mono text-xs">{userId?.substring(0, 8)}...</span>
            </div>
            <button
                    ...
                    <div className="relative p-6">
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex-1">
                          ...
                          <h3 className="text-lg sm:text-xl font-black text-white">
                            {match.team1.name} <span className="text-emerald-400">vs</span> {match.team2.name}
                          </h3>
                        </div>
                      </div>
                      ...
                      {match.status === 'live' && match.team1.score ? (
                        <div className="grid grid-cols-2 gap-4 mb-4 py-4 border-y border-slate-600/50">
                          <div className="text-center">
                            <p className="text-emerald-400 font-bold text-lg">{match.team1.score}</p>
                            <p className="text-xs text-slate-400">{match.team1.overs} Overs</p>
                          </div>
                          <div className="text-center">
                            <p className="text-cyan-400 font-bold text-lg">Target {match.team2.target}</p>
                            <p className="text-xs text-slate-400">{match.team2.overs} Overs</p>
                          </div>
                        </div>
                      ) : (
                        <div className="py-4 border-y border-slate-600/50 mb-4 text-center">
                          <p className="text-slate-400 text-sm">⏰ {match.time || 'Time TBA'}</p>
                        </div>
                      )}

                      <p className="text-sm text-slate-400 mb-4">📍 {match.venue}</p>
